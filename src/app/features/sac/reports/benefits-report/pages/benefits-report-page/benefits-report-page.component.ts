...
// Services & Models
import { BenefitsReportService } from '../../services';
import { BenefitsReportParams, BenefitReportType } from '../../../shared/models';
import {
...
@Component({
  selector: 'app-benefits-report-page',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './benefits-report-page.component.html',
  styleUrl: './benefits-report-page.component.scss'
})
export class BenefitsReportPageComponent implements OnInit {
  private readonly fb = inject(FormBuilder);
  private readonly reportService = inject(BenefitsReportService);
  ...
  // Enum for template
  readonly BenefitReportType = BenefitReportType;
  ...
  private initForm(): void {
    ...
    this.form = this.fb.group({
      ...
      reportType: [BenefitReportType.Benefits, [Validators.required]]
    });
  }
  ...
  generateReport(): void {
    ...
    const params: BenefitsReportParams = {
    ...
    const serviceCall = reportType === BenefitReportType.Benefits
      ? this.reportService.generateBenefitReport(params)
      : this.reportService.generateBipsReport(params);
    ...
    serviceCall.subscribe({
      next: (response) => {
        ...
        try {
          ...
          const reportName = reportType === BenefitReportType.Benefits
            ? 'ReporteBeneficios'
            : 'ReportePuntosBips';
  ...
  clearForm(): void {
    ...
    this.form.patchValue({
      ...
      reportType: BenefitReportType.Benefits
    });
  }
}
