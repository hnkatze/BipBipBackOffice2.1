...
// Services & Models
import { OrdersByDeliveryService } from '../../services/orders-by-delivery.service';
...
/**
 * Componente para gestionar comandas por delivery
 *
 ...
 */
@Component({
  selector: 'app-orders-by-delivery-page',
  templateUrl: './orders-by-delivery-page.component.html',
...
export class OrdersByDeliveryPageComponent implements OnInit {
  private ordersByDeliveryService = inject(OrdersByDeliveryService);
  ...
  loadCities(): void {
    ...
    this.ordersByDeliveryService.getCities()
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (data) => {
  ...
  private loadDriversByCity(cityId: number): void {
    ...
    this.ordersByDeliveryService.getDriversByCity(cityId)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (data) => {
  ...
  exportPDF(): void {
    ...
    this.ordersByDeliveryService.generatePDFReport(fechaInicio, fechaFin, driver.idDriver)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (base64Data) => {
          const fileName = `Reporte Comandas ${driver.fullNameDriver} (${fechaInicio} - ${fechaFin}).pdf`;
  ...
  exportExcel(): void {
    ...
    this.ordersByDeliveryService.generateExcelReport(fechaInicio, fechaFin, driver.idDriver)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (base64Data) => {
          const fileName = `Reporte Comandas ${driver.fullNameDriver} (${fechaInicio} - ${fechaFin}).xlsx`;
