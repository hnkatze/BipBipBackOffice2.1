...
export class TurnOnPromotionEditPageComponent implements OnInit {
  ...
  private loadPromotion(): void {
    ...
    this.turnOnService.getTurnOnPromotionById(+id)
      ...
      .subscribe({
        ...
        error: (error) => {
          console.error('Error loading turn on promotion:', error);
          this.isLoading.set(false);
          this.messageService.add({
            severity: 'error',
            summary: 'Error',
            detail: 'No se pudo cargar la promoción Turn On'
          });
          this.router.navigate(['/notification-managements/in-app-promotions']);
        }
      });
  ...
  private populateForm(promotion: TurnOnPromotionResponse): void {
    // Determinar modo
    const mode = promotion.promocodeRequired !== null
      ? PromotionMode.PromoCode
      : PromotionMode.Discount;

    // Convertir discount value de backend a frontend
    let displayDiscountValue = 0;
    if (promotion.type === TurnOnDiscountType.PercentageDiscount) {
      displayDiscountValue = promotion.value * 100; // 0.15 → 15
    } else {
      displayDiscountValue = promotion.value;
    }

    this.form.patchValue({
      criteriaId: promotion.criteriaId,
      promotionMode: mode,
      discountType: promotion.type,
      discountValue: displayDiscountValue,
      promocodeRequired: promotion.promocodeRequired,
      minimumPurchaseValue: promotion.constraints?.minimumPurchase?.value || 0,
      constraintDateFrom: promotion.constraints?.dateFrom ? new Date(promotion.constraints.dateFrom) : new Date(),
      constraintDateTo: promotion.constraints?.dateTo ? new Date(promotion.constraints.dateTo) : new Date(),
      constraintIsActive: promotion.constraints?.enable || false
    });

    // Actualizar signals
    this.promotionMode.set(mode);
    if (promotion.type !== null) {
      this.selectedDiscountType.set(promotion.type);
    }
  ...
  onSubmit(): void {
    ...
    this.turnOnService.updateTurnOnPromotion(id, formData)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: () => {
          ...
          this.messageService.add({
            severity: 'success',
            summary: 'Éxito',
            detail: 'Promoción Turn On actualizada correctamente'
          });
          setTimeout(() => {
            this.router.navigate(['/notification-managements/in-app-promotions']);
          }, 1500);
        ...
        error: (error) => {
          console.error('Error updating turn on promotion:', error);
          this.isSaving.set(false);
          this.messageService.add({
            severity: 'error',
            summary: 'Error',
            detail: 'No se pudo actualizar la promoción Turn On'
          });
        }
      });
  }
